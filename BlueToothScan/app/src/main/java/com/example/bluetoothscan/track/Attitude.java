...
//		txt1 = (TextView)((Activity)context).findViewById(R.id.text);
	...
	private void postAttitude(String type, float[]attitude, long timestamp){
//		RequestParams params=new RequestParams();
//		params.add("type", type);
//    	params.add("attitude", attitude[0]+","+attitude[1]+","+attitude[2]+","+attitude[3]);
//    	params.add("timestamp", ""+timestamp);
//		client.post("http://139.129.24.47:3000/sendAttitude", params,new AsyncHttpResponseHandler() {
//			@Override
...
//		});
	...
	private void setCompassHeading(float ori){
//		mv.setCompassHeading(ori);
//		txt2.setText(""+(Math.toDegrees(ori)+180));
	}

	private void setHeading(float ori){
		mv.setHeading(ori);
//		txt1.setText(""+(Math.toDegrees(ori)+180));
	}
		...
		synchronized (this) {
			...
			if (event.sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD) {
				...
				if (success) {
					...
					mMatrix = RR;
					postAttitude(TYPE_COMPASS,getQuaternionFromMatrix(mMatrix),time);
					//初始时
...
//				if (mGravity[0] == 0f && mGravity[1] == 0f && mGravity[2] == 0f) {
...
//							* event.values[2];
//				}
//				mAccelerometer[0] = event.values[0];// - mGravity[0];
//				mAccelerometer[1] = event.values[1];// - mGravity[1];
//				mAccelerometer[2] = event.values[2];// - mGravity[2];
